...
def is_packed( pe ):
    """
    Entropy
    address point analysis
    relocation (highly sequential, indicative of obfuscation purpose)
    malformations
    section characteristics
    overlay data
    uncommon values of fields
    ...
    if some of the sections have high entropy it could be an isntaller or a packed file, hard to tell one from the other

    if relocations are found and those go over the entry point and are very continuous or point outside section's boundaries => 
        it might imply that an obfuscation trick is being used or the relocations are corrupt (maybe intentionally)

    if the file has overlay data, that might be supiscious

    if import tables or strings exist (are pointed to) to within the header or in the area betweent the PE header and the first section
        that's supicious

    if we have warnings, that's suspicious, some of those will be because of out-of-ordinary values are found in the PE header fields

    if few symbols are imported and some of those are loadlibrary and getprocaddress, that can be flagged

    if there are few or none (should come with a standard "density" of strings/kilobytes of data) longer (>8) ascii sequences that might indicate packed data, 
        (this is similar to the entropy test in some ways but might help to discard cases of legitimate installer or compressed data)
